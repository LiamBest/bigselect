...
| data-on-change    | functionName               | Function to call when this selects selected content changes, first parameter is an array of the changed values. |
...
| $(element).bigselectexport()                                      | Exports the data for this element, EG: If the name was 'testDataName' and it was a single select, the result is 'array["testDataName"] = "value";' if it is a multi-select the result is 'array["testDataName[]"] = "value1";array["testDataName[]"] = "value2";'. |
...
| $(element).bigselectset(array selectedItems, bool ignoreOnUpdate) | Sets newly selected items for this big select, useful for programmatically updating the selected items. If false is sent for ignoreOnUpdate any update handler assigned to the big select element will be ignored.                                                 |
